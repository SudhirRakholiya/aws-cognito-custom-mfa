...
public class AuthenticationHelper {
	...
	public byte[] getPasswordAuthenticationKey(String userId, String userPassword, BigInteger B, BigInteger salt) {
		...
		final BigInteger s = (B.subtract(KK.multiply(GG.modPow(x, N))).modPow(a.add(u.multiply(x)), N)).mod(N);
	...
	public String getHmac(String userPoolID,AdminInitiateAuthResult challenge, String password) {
		...
		try {
			...
			byte[] dateBytes = DateUtil.getDate().getBytes(StringUtils.UTF8);
			hmac = mac.doFinal(dateBytes);
		} catch (Exception e) {
			System.out.println(e);
		}
		String finalData = new String(Base64.encode(hmac), StringUtils.UTF8);
